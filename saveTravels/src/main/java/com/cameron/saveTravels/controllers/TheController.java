...
public class TheController {
	...
	public String createExpense(@Valid @ModelAttribute("newExpense") Expense expenseInput, BindingResult result,
			Model viewModel) {
		if (result.hasErrors()) {
	...
	@PutMapping("/edit")
	public String editPut(@Valid @ModelAttribute("editExpense") Expense expenseInput, BindingResult result,
			Model model) {
		if (result.hasErrors()) {
	...
	public String showOne(@PathVariable("id") Long id, Model model) {
		model.addAttribute("thisExpense", eService.getById(id));
		return "show.jsp";
	}
}
